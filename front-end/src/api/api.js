...
const ORS_URL = "https://api.openrouteservice.org/geocode/reverse?api_key";
// ORS end points
...
async function convertCoordsToAddress(lat, lng, radius = 1) {
  const ORS_COORDS_ADDRESS_URL = `${ORS_URL}=${API_KEY}&point.lon=${lng}&point.lat=${lat}&boundary.circle.radius=${radius}&size=10&layers=address,street,neighbourhood,locality,venue,region,country&sources=openstreetmap`;
...
export { getAllTaxistands, convertCoordsToAddress };
